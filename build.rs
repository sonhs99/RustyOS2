...
fn main() {
    ...
    //4. ImageMaker
    image_make(&target_dir, "Disk.img");
}

fn build_bootloader(source: &Path, target: &Path) {
...
fn build_kernel(source: &Path, root_target: &Path, arch: &str) {
    println!(
        "cargo:rerun-if-changed={}/arch/{}/src",
        &source.display(),
        arch
    );
    ...
    if arch == "x86" {
        let nasm = Command::new("nasm")
            .arg("-o")
            .arg(format!("{}/EntryPoint.bin", &target.display()))
            .arg(&entry_file[0])
            .status()
            .unwrap();
        assert!(nasm.success());
    ...
    for file in asm_file {
        ...
        let nasm = Command::new("nasm")
            ...
            .arg(nasm_op)
            .arg("-o")
            .arg(format!("{}/{}.o", &target.display(), &ext[0]))
            .arg(&file)
            .status()
            .unwrap();
        assert!(nasm.success());
    ...
    let cargo = Command::new("xargo")
        .current_dir(&src)
        .arg("build")
        // .arg("-Zbuild-std=core,compiler_builtins")
        // .arg("--target")
        // .arg(format!("{}/triple", &source.display()))
    ...
    let ld = Command::new("ld")
        ...
        .arg("-n")
        .arg("-T")
        .arg(format!("{}/linker.ld", source.display()))
        .arg("-o")
...
fn merge_file(file_list: Vec<&str>, output_name: &str) {
...
fn image_make(target: &Path, image_name: &str) {
    ...
    {
        offset[i] = idx;
        let mut file = File::open(&format!("{}/{}", target.display(), file_name)).unwrap();
