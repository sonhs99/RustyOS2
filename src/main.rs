...
fn main() {
    ...
    let sector_counts = merge_kernel(
        vec![
    ...
    println!("----------------FILE LIST------------------");
    println!(
        "BootLoader    : {:2} Sector(s), Offset [{:#04x}]",
        bootloader_sector, 0
    );
    println!(
        "32-bit Kernel : {:2} Sector(s), Offset [{:#04x}]",
        kernel32_sector, bootloader_sector
    );
    println!(
        "64-bit Kernel : {:2} Sector(s), Offset [{:#04x}]",
        kernel64_sector,
        bootloader_sector + kernel32_sector
    );
    println!("----------------SUB TOTAL------------------");
    println!(
        "Total Kernel  : {:2} Sector(s)",
        kernel32_sector + kernel64_sector
    );
    println!("------------------TOTAL--------------------");
    println!(
        "Total         : {:2} Sector(s)",
        bootloader_sector + kernel32_sector + kernel64_sector
    );

    output
        .write_at(5, &u16_to_u8(kernel32_sector + kernel64_sector))
        .unwrap();
    output.write_at(7, &u16_to_u8(kernel32_sector)).unwrap();
}

fn merge_kernel(file_list: Vec<&str>, output: &mut File) -> Vec<u16> {
