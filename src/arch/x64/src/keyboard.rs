...
pub fn IsOutputBufferFull() -> bool {
	if (InPortByte( 0x64 ) & 0x01) != 0 { return true; }
	return false;
}

pub fn IsInputBufferFull() -> bool {
	if (InPortByte( 0x64 ) & 0x02) != 0 { return true; }
	return false;
}
...
pub fn IsUseCombinedCode(ScanCode: u8) -> bool {
	...
	unsafe {
		if IsAlphabetScanCode(DownScanCode) {
			UseCombinedKey = if gs_stKeyboardManager.ShiftDown ^ gs_stKeyboardManager.CapsLockOn { true }
							 else { false }
		} else if IsNumberOrSymbolScanCode(DownScanCode) {
			UseCombinedKey = if gs_stKeyboardManager.ShiftDown { true }
							 else { false }
		} else if IsNumberPadScancode(DownScanCode) && !gs_stKeyboardManager.ExtendedCodeIn {
			UseCombinedKey = if gs_stKeyboardManager.NumLockOn { true }
							 else { false }
		}
	}
...
pub fn UpdateCombinationKeyStatusAndLED(ScanCode: u8) {
	let (Down, DownScanCode) = if ScanCode & 0x80 != 0 { (false, ScanCode & 0x7F) }
							 else { (true, ScanCode) };
	...
	unsafe {
		...
		if LEDStatusChange {
			ChangeKeyboardLED(gs_stKeyboardManager.CapsLockOn, gs_stKeyboardManager.NumLockOn, gs_stKeyboardManager.ScrollLockOn);
		}
	}
}

pub fn ConvertScanCodeToASCIICode(
	ScanCode: u8, 
	ASCIICode: &mut u8, 
	Flags: &mut u8
) -> bool {
	...
	if ScanCode == 0xE1 {
		...
		*Flags = KeyStatement::KeyFlagsDown as u8;
		unsafe { gs_stKeyboardManager.SkipCountForPause = KEY_SKIPCOUNTFORPAUSE; }
		return true;
	} else if ScanCode == 0xE0 {
		unsafe { gs_stKeyboardManager.ExtendedCodeIn = true; }
		return false;
	}

	if IsUseCombinedCode(ScanCode) {
		*ASCIICode = KeyMappingTable[ (ScanCode & 0x7F) as usize ].CombinedCode();
	} else { *ASCIICode = KeyMappingTable[ (ScanCode & 0x7F) as usize ].NormalCode();}
